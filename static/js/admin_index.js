...
function exChange(data) {
  $("#Tbanner_image").css(
    "background-image",
    `url( ${HOST_URL_TPLANET_DAEMON}${data["banner-image"]})`
  );

  if (`${data["t-planet-img"]}`.localeCompare("undefined") === -1) {
    $("#t_planet_img").attr(
      "src",
      `${HOST_URL_TPLANET_DAEMON}${data["t-planet-img"]}`
    );
  }

  $("#csr_img").css(
    "background-image",
    `url( ${HOST_URL_TPLANET_DAEMON}${data["csr-img"]})`
  );
  $("#sdg_img").css(
    "background-image",
    `url( ${HOST_URL_TPLANET_DAEMON}${data["sdg-img"]})`
  );
  $("#twins_img").css(
    "background-image",
    `url( ${HOST_URL_TPLANET_DAEMON}${data["twins-img"]})`
  );
...
export async function mockup_new() {
  ...
  if (
    document
      .getElementById("Tbanner_image")
      .style.backgroundImage.includes("data:") == true
  ) {
    var Tbanner_image = document
      .getElementById("Tbanner_image")
      .style.backgroundImage.replace('url("', "");
    Tbanner_image = document
      .getElementById("Tbanner_image")
  ...
  if (document.getElementById("t_planet_img").src.includes("data:") == true) {
    formData.append(
      "t-planet-img",
      DataURIToBlob(document.getElementById("t_planet_img").src)
    );
  }

  if (
    document
      .getElementById("csr_img")
      .style.backgroundImage.includes("data:") == true
  ) {
    var csr_img = document
      .getElementById("csr_img")
      .style.backgroundImage.replace('url("', "");
    csr_img = document
      .getElementById("csr_img")
  ...
  if (
    document
      .getElementById("sdg_img")
      .style.backgroundImage.includes("data:") == true
  ) {
    var sdg_img = document
      .getElementById("sdg_img")
      .style.backgroundImage.replace('url("', "");
    sdg_img = document
      .getElementById("sdg_img")
  ...
  if (
    document
      .getElementById("twins_img")
      .style.backgroundImage.includes("data:") == true
  ) {
    var twins_img = document
      .getElementById("twins_img")
      .style.backgroundImage.replace('url("', "");
    twins_img = document
      .getElementById("twins_img")
